...
def test_get_zipfile(get_repo_url):
    ...
    assert ghwm.zip_file_status == ZipStatusChoices.PENDING
    ...
    assert ghwm.zip_file_status == ZipStatusChoices.FAILURE

    ghwm2 = GitHubWebhookMessageFactory()
    assert ghwm2.zip_file_status == ZipStatusChoices.PENDING
    ...
    assert ghwm2.zip_file_status == ZipStatusChoices.SUCCESS
