...
from grandchallenge.core.views import get_data_folder_path, get_dirnames


def get_public_results(request, challenge_short_name):
    """ Return an array of strings respresenting all results that can be
    publically viewed.
    """
    try:
        ...
        return HttpResponse(
            "Cannot list public results. Public results should be in folder "
            "'results/public/' but this folder could not be found"
        )
...
def get_public_results_by_challenge_name(challenge_short_name):
    """ Made a separate method here to be able to also call this API function
    from within other parts of the code.
