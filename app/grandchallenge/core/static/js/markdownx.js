    ...
    1: [
      function (require, module, exports) {
        ...
        var UPLOAD_URL_ATTRIBUTE = "data-markdownx-upload-urls-path",
          ...
          LATENCY_MINIMUM = 500,
          XHR_RESPONSE_ERROR = "Invalid response",
          UPLOAD_START_OPACITY = "0.3",
          NORMAL_OPACITY = "1";
        var EventHandlers = {
        ...
        var keyboardEvents = {
          ...
          handlers: {
            applyTab: function (properties) {
              return (
                properties.value.substring(0, properties.start) +
                (properties.value
                  ...
                  : properties.value
                      .substring(properties.start, properties.end)
                      .replace(/^/gm, "\t") +
                    properties.value.substring(properties.end))
              );
            },
            removeTab: function (properties) {
              ...
              if (properties.start === properties.end) {
                properties.start =
                  properties.start > 0 &&
                  properties.value[properties.start - 1].match(/\t/) !== null
                    ? properties.start - 1
                    : properties.start;
              ...
              return (
                properties.value.substring(0, properties.start) + substitution
              );
            },
            ...
            applyDuplication: function (properties) {
              if (properties.start !== properties.end)
                return (
                  properties.value.substring(0, properties.start) +
                  properties.value.substring(properties.start, properties.end) +
                  (~properties.value
            ...
            switch (event.key) {
              case this.keys.TAB:
                return event.shiftKey
                  ? this.handlers.removeTab
                  : this.handlers.applyTab;

              case this.keys.DUPLICATE:
                return event.ctrlKey || event.metaKey
                  ? this.handlers.applyDuplication
                  : false;

              case this.keys.INDENT:
                return event.ctrlKey || event.metaKey
                  ? this.handlers.applyIndentation
                  : false;

              case this.keys.UNINDENT:
                return event.ctrlKey || event.metaKey
                  ? this.handlers.removeIndentation
                  : false;
        ...
        var MarkdownX = function (parent, editor, preview) {
          ...
          var _initialize = function () {
            ...
            properties._latency = Math.max(
              parseInt(properties.editor.getAttribute(LATENCY_ATTRIBUTE)) || 0,
              LATENCY_MINIMUM,
            );
            properties._editorIsResizable =
              (
                properties.editor
                  .getAttribute(RESIZABILITY_ATTRIBUTE)
                  .match(/true/i) || []
              ).length > 0 &&
          ...
          var inputChanged = function () {
            properties.editor = properties._editorIsResizable
              ? updateHeight(properties.editor)
              : properties.editor;
            return _markdownify();
          };
          ...
          var sendFile = function (file) {
            ...
            var xhr = new utils_1.Request(
              properties.editor.getAttribute(UPLOAD_URL_ATTRIBUTE),
            ...
            xhr.error = function (response) {
              ...
              utils_1.triggerCustomEvent("fileUploadError", properties.parent, [
                response,
              ]);
              insertImage(XHR_RESPONSE_ERROR);
              properties.editor.style.opacity = NORMAL_OPACITY;
          ...
          var getMarkdown = function () {
            var xhr = new utils_1.Request(
              properties.editor.getAttribute(PROCESSING_URL_ATTRIBUTE),
              utils_1.preparePostData({
          ...
          var insertImage = function (textToInsert) {
            properties.editor.value =
              "" +
              properties.editor.value.substring(
                0,
            ...
            properties.editor.selectionStart = properties.editor.selectionEnd =
      ...
      function (require, module, exports) {
        ...
        function mountEvents() {
          ...
          return collections.map(function (events) {
            return events.listeners.map(function (series) {
              return events.object.addEventListener(
                series.type,
                series.listener,
                series.capture,
              );
        ...
        function addClass(element) {
          ...
          className.map(function (cname) {
            if (element.classList) element.classList.add(cname);
            else {
        ...
        function removeClass(element) {
          ...
          className.map(function (cname) {
            if (element.classList) element.classList.remove(cname);
            else {
              var classes = element.className.split(" "),
                idx = classes.indexOf(cname);
