...
def test_get_component_interface_values_for_user():
    ...
    assert list(
        get_component_interface_values_for_user(user=user).order_by("pk")
    ) == [civ1, civ3, civ5, civ7, civ9, civ11, civ13, civ15]

    # subset by interface
    assert list(
        get_component_interface_values_for_user(
            user=user, interface=ci1
        ).order_by("pk")
    ) == [civ1, civ3, civ5, civ7]
    assert list(
        get_component_interface_values_for_user(
            user=user, interface=ci2
        ).order_by("pk")
    ...
    assert (
        list(
            get_component_interface_values_for_user(user=user, civ_pk=civ2.pk)
        )
        == []
    )
