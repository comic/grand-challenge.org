...
def test_parse_structured_logs_filters_task():
    ...
    parsed_log = parse_structured_log(structured_log=log_with_task, task="1")

    assert parsed_log.message == "message with task"

    parsed_log = parse_structured_log(
        structured_log=log_without_task, task="1"
    )

    assert parsed_log is None

    parsed_log = parse_structured_log(structured_log=log_with_task, task=None)

    # when `task` is None, messages with a task are __included__
    assert parsed_log.message == "message with task"

    parsed_log = parse_structured_log(
        structured_log=log_without_task, task=None
    )
