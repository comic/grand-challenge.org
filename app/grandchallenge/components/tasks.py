...
@acks_late_micro_short_task
@transaction.atomic
def update_job_requirements_and_credits(*, pk):
    from grandchallenge.algorithms.models import Job

    job = Job.objects.prefetch_related("algorithm_image").get(pk=pk)

    job.requires_gpu_type = job.algorithm_image.requires_gpu_type
    job.requires_memory_gb = job.algorithm_image.requires_memory_gb
...
@acks_late_micro_short_task
@transaction.atomic
def update_submission_algorithm_limits(*, pk):
    from grandchallenge.evaluation.models import Submission

    submission = Submission.objects.prefetch_related("algorithm_image").get(
        pk=pk
    )

    if submission.algorithm_image:
        submission.algorithm_requires_gpu_type = (
            submission.algorithm_image.requires_gpu_type
        )
        submission.algorithm_requires_memory_gb = (
            submission.algorithm_image.requires_memory_gb
        )
    else:
        submission.algorithm_requires_gpu_type = ""
        submission.algorithm_requires_memory_gb = 0
...
@acks_late_micro_short_task
@transaction.atomic
def update_evaluation_requirements(*, pk):
    from grandchallenge.evaluation.models import Evaluation

    evaluation = Evaluation.objects.prefetch_related("method").get(pk=pk)

    evaluation.requires_gpu_type = evaluation.method.requires_gpu_type
    evaluation.requires_memory_gb = evaluation.method.requires_memory_gb
    evaluation.save()
