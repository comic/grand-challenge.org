...
@pytest.mark.django_db
def test_public_is_set():
    res = AlgorithmResultFactory()

    assert res.job.public is False

    res.public = True
    res.save()
    res.refresh_from_db()

    assert res.job.public is True
...
@pytest.mark.django_db
def test_comment_is_set():
    res = AlgorithmResultFactory()

    assert res.job.comment == ""

    res.comment = "foo"
    res.save()
    res.refresh_from_db()

    assert res.job.comment == "foo"

    res.comment = "bar"
    res.save()
    res.refresh_from_db()

    assert res.job.comment == "bar"
