...
function renderResultViewerGUI(base,restricted) {
    restricted = typeof(restricted) != 'undefined' ? restricted : true; //restrict the viewer by default
    ...
    row1.append(createCheckboxElement("showOrg", true, "show original scan"));    
    row1.append(createCheckboxElement("showRes", true, "show current result: " + $("#resultDir").val()));    
    row1.append(createLabel("method1", "show additional results:"));    
    row1.append(createStringDropdownBoxAndBreak("method1",$.merge(["None"],getAllResultsExceptCurrent())));    
    row1.append(createStringDropdownBoxAndBreak("method2",$.merge(["None"],getAllResultsExceptCurrent())));
    row1.append(createCheckboxElement("showAll", false, "show all results"));    

    var row2 = $("<div>");    
    row2.append(createLabel("Item", "Item"));
    row2.append(createStringDropdownElement("Item",["DenseAbnormalities.png",
                                                    "Fibrosis.png",
    ...
    row3.append(createLabel("Width", "Image width in pixels"));
    row3.append(createIntSelectBox("Width", 200));
    row3.append($("<br/>"));
    row3.append(createLabel("ShowHeaders", "Show info headers for each image"));
    row3.append(createCheckBox("ShowHeaders", true));
    row3.append($("<br/>"));


    base.append(createSingleRowTable([row1, row2, row3]));
    base.append(createSubmitButton("loadButton", "Load"));
}


//return a table with a single row, each element in elements in its own column
function createSingleRowTable(elements) {
    var table = $("<table>");
    var tr = $("<tr>");
    tr.appendTo(table);
...
function createCheckboxElement(id, checked, labelText) {
    var containerDiv = $("<div>");
    containerDiv.attr("class", "editor-label");

    var checkBox = createCheckBox(id, checked);
...
function createIntSelectBox(id, startVal) {
...
function createIntDropdownBox(id, options) {
...
//create this:
//<select>  <option>options[0]</option>... </select>  
function createStringDropdownBox(id, options) {
    var selectBox = $('<select>');
...
function createSubmitButton(id, buttonText) {
...
//</div>
function createIntSelectElement(id, startVal) {
    var prevButton = createSubmitButton("prev" + id + "Button", "prev");
    var textBox = createIntSelectBox(id, startVal);    
    var nextButton = createSubmitButton("next" + id + "Button", "next");
...
function createIntDropdownElement(id, options) {
    var prevButton = createSubmitButton("prev" + id + "Button", "prev");    
...
function createStringDropdownBoxAndBreak(id, options) {    
    var textBox = createStringDropdownBox(id, options);    
    var containerDiv = $("<div>");
    containerDiv.attr("class", "editor-field");    
    containerDiv.append(textBox);
...
function createStringDropdownElement(id, options) {
    var prevButton = createSubmitButton("prev" + id + "Button", "prev");    
    var textBox = createStringDropdownBox(id, options);
    var nextButton = createSubmitButton("next" + id + "Button", "next");
    var containerDiv = $("<div>");
...
function getAllResultsExceptCurrent(){
    all = getAllResults();
...
function loadAllScreenshots() {
    ...
    $.each(params["resultDirs"], function (index, value) {
        ...
        image.html("<img src=\"../../Content/images/ajax-loader.gif\">");  
